...
/* Buttons style (Streamlit default buttons are targeted like this) */
.stButton>button {
...
/* Emergency button wrapper: stronger pink */
.emergency .stButton>button {
    background: var(--pink) !important;
    color: var(--deep-dark) !important;
    box-shadow: 0 6px 22px rgba(252,62,87,0.25);
}

.emergency .stButton>button:hover {
...
.status-badge {
    ...
    padding: 6px 10px;
    border-radius: 999px;
    background: linear-gradient(90deg, rgba(252,62,87,0.95), rgba(94,145,138,0.95));
    color: var(--deep-dark);
    font-weight: 800;
}
...
st.markdown('<div class="faint">Last detection</div>', unsafe_allow_html=True)
...
if st.session_state.last_capture and os.path.exists(st.session_state.last_capture):
    ...
    st.image(img, width=380, caption="Latest Capture")
else:
